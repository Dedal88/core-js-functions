...
function getArgumentsCount(funcs) {
  const total = [];
  funcs.map((item) => total.push(item.length));
  return total;
}
...
function getPolynom(...args) {
  ...
  return (x) => {
    return args.reduce(
      (acc, item, index) => acc + item * x ** (args.length - 1 - index),
      0
    );
  };
}
...
function logger(func, logFunc) {
  return (...args) => {
    const stringArgs = args.map((item) => JSON.stringify(item)).join(',');
